...
class BasicCNNModel:
    ...
    def create_model(self):
        ...
        char_features = tf.concat([max_pooled_char_maps, char_lexical_features], axis=1)
        word_features = tf.concat([max_pooled_word_maps, word_lexical_features], axis=1)

        # dropout_features = features
        dropout_char_features = tf.layers.dropout(char_features, training=is_train)
        dropout_word_features = tf.layers.dropout(word_features, training=is_train)

        features = tf.concat([dropout_char_features, dropout_word_features], axis=1)
        ...
        reg_logit = reg_output_layer(features)
        cls_logit = cls_output_layer(features)
        ...
        clipped_optimizer = tf.contrib.opt.VariableClippingOptimizer(basic_optimizer,clip_dict,1)
        '''
        ...
        for batch in data.next_test_batch(1000):
            ...
            for data_key, reg_label, cls_label, reg_prob, cls_prob in zip(batch['keys'], reg_labels, cls_labels,
                ...
                if reg_label != 6:
                    ...
                    # print (doc_id,sent_id,offset,length,data.id2label[cls_label])
...
if __name__ == "__main__":
    ...
    # dev_ret_path = "tmp/"
    model = BasicCNNModel(conf=confs)
